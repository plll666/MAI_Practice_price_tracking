...
async def get_user_contacts(
    ...
    logger.info(f"Контактные данные пользователя '{current_user.login}' найдены: email={contacts.email is not None}, phone={contacts.phone_number is not None}, tg={contacts.tg is not None}")
    return contacts
...
async def update_user_contacts(
    ...
    if old_contacts:
        ...
        if old_contacts.phone_number != contacts.phone_number:
            changes.append(f"phone: '{old_contacts.phone_number or 'не указан'}' -> '{contacts.phone_number or 'не указан'}'")
